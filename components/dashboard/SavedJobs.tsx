...
const SavedJobs: React.FC<SavedJobsProps> = ({ savedJobs, onViewJobDetails, onViewCompanyProfile }) => {
  const { savedJobs: localSavedJobs } = useSavedJobsStore();
  const displayJobs = localSavedJobs.length > 0 ? localSavedJobs : savedJobs;
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {displayJobs.length > 0 ? (
            ...
            <div className="py-8 text-center text-gray-500">
                ...
                <p className="mt-1">Click the Save button on a job to save it for later.</p>
            </div>
        )}
    </div>
  );
